...
Plateau::Plateau()
{
_plateau({nullptr}); //On remplit le vecteur de pointeur null 
}

Plateau::~Plateau()
{
  for (int i = 0; i < _joueurs.size(); i++) delete _joueurs[i];
  for (int i = 0; i < _plateau.size(); i++) delete _plateau[i];
}
...
void Plateau::initPlateau()
{
  ...
  for(int y=0; y<=nbJoueurs; i++)
  {
    cin>> Pseudo;
    this->_joueurs.push_back(Joueur(Pseudo, i, this->_plateau[0], this));
  } 
